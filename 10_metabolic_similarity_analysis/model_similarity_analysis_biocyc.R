...
ggplot(similarity1, aes(x=genome_similarity, y=similirity) ) +
  ...
  theme(axis.text=element_text(size=12, family="Arial"), axis.title=element_text(size=16, family="Arial")) +
  theme(legend.title = element_text(size=16), legend.text = element_text(size=16))
ggsave(out <- paste('result/','Genome_VS_model_similarity_biocyc','.eps', sep = ""), width=5, height=4, dpi=600)
...
ggplot(similarity1, aes(x=EVO_SCORE, y=similirity) ) +
  ...
  theme(axis.text=element_text(size=12, family="Arial"), axis.title=element_text(size=16, family="Arial")) +
  theme(legend.title = element_text(size=16), legend.text = element_text(size=16))
ggsave(out <- paste('result/','EVO_SCORE_VS_model_similarity_biocyc','.eps', sep = ""), width=5, height=4, dpi=600)
...
#fit linear model
linearModel <- lm(similirity ~ EVO_SCORE, data=similarity1)
...
similarity1$EVO_SCORE2 <- similarity1$EVO_SCORE^2

#fit quadratic regression model
quadraticModel <- lm(similirity ~ EVO_SCORE + EVO_SCORE2, data=similarity1)
...
#fit linear model
linearModel <- lm(similirity ~ genome_similarity, data=similarity1)
...
#create a new variable for hours2
similarity1$genome_similarity2 <- similarity1$genome_similarity^2

#fit quadratic regression model
quadraticModel <- lm(similirity ~ genome_similarity + genome_similarity2, data=similarity1)
...
ggplot(similarity1, aes(x=EVO_SCORE, y=trait_similarity) ) +
  geom_hex(bins = 120) +
  scale_fill_continuous(type = "viridis") +
  geom_smooth(method = "gam") +
  theme_bw() +
  ...
  theme(axis.text=element_text(size=12, family="Arial"), axis.title=element_text(size=16, family="Arial")) +
  theme(legend.title = element_text(size=16), legend.text = element_text(size=16))
ggsave(out <- paste('result/','EVO_SCORE_VS_trait_similarity','.eps', sep = ""), width=5, height=4, dpi=600)

cor.test(similarity1$EVO_SCORE, similarity1$trait_similarity)



ggplot(similarity1, aes(x=genome_similarity, y=trait_similarity) ) +
  geom_hex(bins = 120) +
  scale_fill_continuous(type = "viridis") +
  geom_smooth(method = "gam") +
  theme_bw() +
  ...
  theme(axis.text=element_text(size=12, family="Arial"), axis.title=element_text(size=16, family="Arial")) +
  theme(legend.title = element_text(size=16), legend.text = element_text(size=16))
ggsave(out <- paste('result/','Genome_VS_trait_similarity','.eps', sep = ""), width=5, height=4, dpi=600)

cor.test(similarity1$genome_similarity, similarity1$trait_similarity)



ggplot(similarity1, aes(x=genome_similarity, y=distance) ) +
  geom_hex(bins = 120) +
  scale_fill_continuous(type = "viridis") +
  geom_smooth(method = "gam") +
  theme_bw() +
  ...
  theme(axis.text=element_text(size=12, family="Arial"), axis.title=element_text(size=16, family="Arial")) +
  theme(legend.title = element_text(size=16), legend.text = element_text(size=16))
ggsave(out <- paste('result/','Genome_similarity_VS_Evolutionary_distance','.eps', sep = ""), width=5, height=4, dpi=600)


cor.test(similarity1$genome_similarity, similarity1$distance)
