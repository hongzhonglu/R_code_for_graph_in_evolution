...
similarity_file <- 'data_for_GEMs_similarity/combined_model_similirity.txt'
similarity_file <- 'data_for_GEMs_similarity/manual_model_similirity.txt'
similarity_file <- 'data_for_GEMs_similarity/manual_model_similirity_remove_rxn_with_no_gene.txt'
...
ggplot(similarity2, aes(x=similirity, y=trait_similarity) ) +
  ...
  theme(axis.text=element_text(size=12, family="Arial"), axis.title=element_text(size=16, family="Arial")) +
  theme(legend.title = element_text(size=16), legend.text = element_text(size=16))
ggsave(out <- paste('result/','kegg_model_VS_trait_similarity','.eps', sep = ""), width=5, height=4, dpi=600)
...
ggplot(similarity1, aes(x=EVO_SCORE, y=similirity) ) +
  ...
  theme(axis.text=element_text(size=12, family="Arial"), axis.title=element_text(size=16, family="Arial")) +
  theme(legend.title = element_text(size=16), legend.text = element_text(size=16))
ggsave(out <- paste('result/','EVO_SCORE_VS_model_similarity_kegg','.eps', sep = ""), width=5, height=4, dpi=600)


cor.test(similarity1$EVO_SCORE, similarity1$similirity)
write.table(similarity1,"result/model_similarity_analysis.txt")
