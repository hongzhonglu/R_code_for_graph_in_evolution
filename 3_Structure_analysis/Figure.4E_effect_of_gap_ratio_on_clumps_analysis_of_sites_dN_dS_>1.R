...
# add the result from experimental pdb files
file_general <- "data/CLUMPS from pdb_homo for dN_dS_max_gap_ratio_"
file_exp <- "data/CLUMPS from pdb_ex for dN_dS_max_gap_ratio_"
parameter <- c("0.2", "0.3", "0.5", "0.6", "0.7", "0.8")
parameter_as_col <- paste("gap_ratio_", parameter, sep="")
unqiue_protein_filter_0.05 <- c()
for (i in parameter){
  ...
  result_general <- read.table(input_dir, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  result_general$p_value <- as.numeric(result_general$p_value)
  result_general0 <- result_general %>% filter(.,p_value <=0.05)
  
  input_dir2 <- paste(file_exp,i, "/pdb_Ex_for_fast_evolution.txt",sep = "")
  result_exp <- read.table(input_dir2, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  result_exp$p_value <- as.numeric(result_exp$p_value)
  result_exp0 <- result_exp %>% filter(.,p_value <=0.05)
  
  unique_protein_num <- length(unique(c(result_general0$locus, result_exp0$locus)))
  unqiue_protein_filter_0.05 <- c(unqiue_protein_filter_0.05, unique_protein_num)
}
effects_ratio2 <- data.frame(gap_ratio = parameter, num_protein = unqiue_protein_filter_0.05)


ggplot(data=effects_ratio2, aes(x=gap_ratio, y=num_protein)) +
  geom_bar(stat="identity", color="blue",fill="blue", alpha=0.3) +
  theme(panel.background = element_rect(fill = "white", color="black", size = 1),
        plot.margin = margin(1, 1, 1, 1, "cm")) +
  labs(x="Gap frequency", y="Number of proteins")+
